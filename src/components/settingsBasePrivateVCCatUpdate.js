module.exports = async function(client, con, interaction, data, language) {
    await interaction.reply({ content: language.components.settingsBasePrivateVCCatUpdate.please, ephemeral: true }).catch(function(e) { if(client?.config?.debugmode) console.log(e) });
    ...
    collector.on('collect', async (m) => {
        ...
        input = input.replaceAll("`", "");
        await con.query(`UPDATE guilds SET privatecallcategory="${input}" WHERE guildid="${interaction.guild.id}"`, async (err, row) => {
            if(err) throw err;
            await m.delete().catch(function(e) { if(client?.config?.debugmode) console.log(e) });
            interaction.editReply({ content: language.components.settingsBasePrivateVCCatUpdate.updated, ephemeral: true })
            collector.stop();
            return;
        });
    });
