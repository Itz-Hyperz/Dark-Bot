module.exports = async function(client, con, interaction, data, language) {
    ...
    await interaction.reply({ content: language.vcRemoveMemberText }).catch(function(e) { if(client?.config?.debugmode) console.log(e) });
    con.query(`SELECT * FROM privatecalls WHERE uniqueid="${channelid}"`, async (err, row) => {
        ...
        collector.on('collect', async (m) => {
            ...
            let allowed = JSON.stringify(newArray);
            ...
            await con.query(`UPDATE privatecalls SET alloweduserids='${allowed}' WHERE uniqueid='${channelid}'`, async (err, row) => {
                if(err) throw err;
                interaction.editReply({ content: language.vcRemoveSuccess })
                collector.stop();
                return;
            });
        });
