module.exports = async function(client, con, interaction, data, language) {
    await con.query(`SELECT * FROM perms WHERE guildid="${interaction.guild.id}" AND permtype="applications" OR guildid="${interaction.guild.id}" AND permtype="admin"`, async (err, row) => {
        ...
        .addComponents(
            ...
            .setLabel(`${language.components.appDeny.denied}: ${interaction.user.tag}`)
            .setStyle('DANGER')
        ...
        await con.query(`SELECT * FROM applications WHERE guildid='${interaction.guild.id}' AND uniqueid='${uniqueid}'`, async (err, row) => {
            if(err) throw err;
            if(!row[0]) return interaction.reply({ content: language.components.appDeny.unable, ephemeral: true }).catch(function(e) { if(client?.config?.debugmode) console.log(e) });
            let notice = new client.discord.MessageEmbed()
            .setColor(data.themecolor || '#FFFFFF')
            .setTitle(language.components.appDeny.title)
            .setDescription(`${language.components.appDeny.app} ${app}\n${language.components.appDeny.message} ${row[0]?.denyMessage}`)
            .setThumbnail(interaction.guild.iconURL({ dynamic: true }))
            .setFooter({ text: `${language.components.appDeny.denied} ${interaction.user.tag}`, iconURL: interaction.user.avatarURL({ dynamic: true }) })
