module.exports = async function(client, con, interaction, data, language) {
    ...
    await con.query(`SELECT * FROM filtered WHERE guildid='${interaction.guild.id}'`, async (err, row) => {
        ...
        .setTitle(language.components.settingsSecurityFilterBack.a)
        .setDescription(`${language.components.settingsSecurityFilterBack.b}\n\`\`\`\n${row[curr]?.content || language.components.settingsSecurityFilterBack.c}\n\`\`\``)
        .setTimestamp()
        .setFooter({ text: `${curr}` })
