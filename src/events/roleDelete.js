module.exports = async(client, con, role) => {
    ...
    const AuditLogFetch = await role.guild.fetchAuditLogs({limit: 1, type: "ROLE_DELETE"}).catch(e => {});
    ...
    await con.query(`SELECT * FROM guilds WHERE guildid='${role.guild.id}'`, async (err, row) => {
        ...
        .setAuthor({ name: `${language.logging.actionLogs} - ${language.events.roles.deleted}`, iconURL: client.user.displayAvatarURL() })
        .addFields(
