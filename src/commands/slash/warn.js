exports.run = async function(client, con, interaction, data, language) {
    await con.query(`SELECT * FROM perms WHERE guildid="${interaction.guild.id}" AND permtype="mod" OR guildid="${interaction.guild.id}" AND permtype="admin"`, async (err, row) => {
        ...
        .setAuthor({ name: `${language.logging.actionLogs} - ${language.logging.types.userWarned}`, iconURL: client.user.avatarURL({ dynamic: true }) })
        .addFields(
        ...
        .setTimestamp()
        await client.utils.makeCase(client, con, data, user.id, interaction.user.id, `${language.logging.types.userWarned}`, reason, logembed)
        await user.send({ embeds: [logembed] }).catch(e => {});
        await interaction.reply({ content: `**${language.logging.types.userWarned}!**`, ephemeral: true }).catch(e => {});
    });
};
